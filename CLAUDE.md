...
4. Background calls `dispatchOutput(markdown, metadata, mode)` — `mode` overrides the `outputMode` pref so each click is explicit per-action
...
3. User clicks Copy/Save (or Enter/C/S) → ElementPicker invokes `onCopy`/`onSave` → Content Script converts and sends `selectionComplete` with `{ markdown, metadata, mode }` → Background
4. Background dispatches; the picker stays active so the user can fire the other action on the same set. X icon / Esc / popup Cancel hard-exits.
...
- **Output dispatch** — `dispatchOutput(markdown, metadata, modeOverride)` in background routes to `copyToClipboard()` or `saveAsFile()`. The optional `modeOverride` ('clipboard' | 'file') wins over the `outputMode` pref when supplied. Single chokepoint for all output paths.
