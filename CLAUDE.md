...
| `src/utils/preferences.js` | Preferences wrapper around `chrome.storage.local` (outputMode, includeMetadata, formatting options, filename template + style) |
...
- **Preferences** stored in `chrome.storage.local`. `Preferences.get()` merges stored values with defaults (`outputMode: 'clipboard'`, `includeMetadata: true`, formatting options: `headingStyle`, `bulletListMarker`, `codeBlockStyle`, `linkStyle`, plus filename options: `filenameTemplate: '{title} - {date}'`, `filenameStyle: 'preserve'`). Shared by popup, options page, and background via webpack. Formatting options flow: background reads prefs → passes in message options → content script calls `converter.applyFormattingOptions()` → Turndown services updated before conversion.
