...
**Status:** Phases 1–4 complete, Phase 5.2–5.3 complete, Phase 5.4–5.6 partially complete (store text drafted, screenshots/submission remaining), Phase 6.1–6.2, 6.5 complete. Phase 6.3–6.4 (Selenium e2e) planned. General actions (full-page conversion, selective element conversion), output options (clipboard/file), site actions via site module registry (X/Twitter, Claude, Grok), redesigned settings/options page (two-column layout with sticky live-preview pane, Apple-style segmented controls, persistent "Available tokens" + "Advanced — Markdown syntax" disclosures, two-step armed reset with toast), output preferences (image handling, link handling, tracking-param strip, page-info format with Inline / Frontmatter shapes, auto-close popup), formatting preferences, customizable filename templates (token-based, Moment-style date formatting), a content-first popup picker (Page content + supported-site content types as rows; Copy / Save buttons in the footer; remembers last-picked content type per site), and a redesigned on-page selection mode (sticky banner with Default Copy/Save toggle, phantom capture layer that fully suppresses host-page `:hover` and link/text interactions, floating action bar with Clear / secondary / primary tiers + X exit; selection persists across actions so Copy and Save can both fire on the same set). GFM output (tables, strikethrough, task lists) via `turndown-plugin-gfm`. Site-action filename titles (each formatter exposes `filenameTitle(contentType, data)` so X tweets save as `X Post by @user`, articles use `twitter-article-title`, Claude/Grok use `Claude — {title}` / `Grok — {title}`). Filename template engine supports pipe-filter syntax (`{title|max:50|default:Untitled}`) with a 100-char default cap on `{title}` to keep filenames sane on long-title pages. Popup shows site-action rows only when the URL path actually matches a content type (per-content-type `pathPatterns` regex array — e.g. `x.com/home` and similar render Page-content-only). X tweet/thread polish complete: long-form tweet body preserves emojis and inline links (mentions become `[@user](https://x.com/user)`, hashtags become `[#Tag](https://x.com/hashtag/Tag)`, posted URLs unmask t.co to display-text destination with truncated-display fallback), verified author indicator (`✓` after display name), engagement summary in X visual order (replies → reposts → likes → bookmarks → views) with bookmarks/views captured, timestamps in viewer-local time matching X's on-page display, and Community Notes (`birdwatch-pivot`) rendered as a labelled blockquote between content and engagement footer. Inline page-info header shows date as `April 29, 2026 at 11:35 AM` (matches the tweet/article timestamp format); YAML frontmatter date stays sortable `2026-04-29 11:35`. X site module hardened for non-English locales: testid-primary engagement extraction with positional summary fallback, structural community-note body detection (no phrase matching), structural video-label stripping. Quote-tweet support: `[role="link"][tabindex="0"]` wrapper containing a nested `User-Name` identifies the quoted post; outer-tweet author/text/timestamp/media extraction excludes that subtree so the focal fields don't silently inherit the quoted tweet's values (the quoted `<time>` always appears first in DOM order). Quote bodies and Community Notes — both multi-line — get per-line `>` blockquote prefixing so source links and trailing paragraphs stay inside the callout. Link-preview cards (`card.wrapper`) render as `🔗 [**title**](t.co URL) — domain` followed by the OG image; the card image is excluded from `_extractMedia` so it doesn't double-list. "Show more" auto-expansion: site modules can expose an optional async `prepareForExtraction(contentType, doc, url)` hook that runs before `extract()`; X uses it to click `[data-testid="tweet-text-show-more-link"]` buttons in scope (focal tweet, or all chain tweets in a thread) and wait for the buttons to disappear (5s budget) so long-tweet bodies are captured in full. Thread extraction is now contiguous same-author only — `_collectThreadChain` walks forward and backward from the focal, skipping empty article wrappers (X's reply-composer chrome) but stopping at the first different-author tweet, so the author's replies-to-commenters in the discussion section don't bleed into thread output. URL-shaped link labels with a trailing `…` (X's visual truncation indicator on long URLs) get the ellipsis stripped — clean labels in markdown, t.co fallback path keeps its `…` since the ellipsis is meaningful chrome there. 708 unit tests passing (18 suites) + 30 integration tests (6 suites). CI via GitHub Actions on every push/PR.
...
| `src/utils/filename-template.js` | Pure-function filename templater — token expansion with pipe-filter syntax (`{title\|max:50\|default:Untitled}`), Moment-style date formatting, preserve/kebab/snake style transforms, sanitization, truncation. 100-char default cap on `{title}` to keep filenames sane on long-title pages (X tweet OG titles, etc.) |
...
- **i18n-safe selectors and detection** — Site modules, detection logic, extractors, formatters, and any per-site code must be locale-stable by default. The user base is global; English-phrase matching silently breaks for every non-English viewer. Always prefer:
  ...
  2. **Structural / positional rules** — first/last child, role attributes (`role="link"`, `role="button"`), tag names, parent/sibling relationships. Use these when there's no testid.
  3. **Locale-invariant anchors** — brand names ("Grok", "Claude"), URL paths (`/hashtag/`, `/status/`), separator characters (`|`, `…`, U+2060), and Unicode shape rules. These don't translate.
  ...
  5. **Phrase matching is a last resort.** When unavoidable (e.g., extracting a localized speaker name like "Shared by {X}"), keep the phrase narrowly scoped, document it inline with an `i18n note:` comment explaining the limitation, and add a followup entry to `private/PLAN.md` → Future Ideas → "i18n hardening followups". Never use phrase matching to *gate* extraction (skip/include decisions) — it must only inform optional cosmetic behavior.

  Existing references for the patterns: `src/sites/x/x-extractor.js` `_extractEngagement` (testid + summary positional), `_extractCommunityNote` (position + role-based footer detection), `_stripVideoLabels` (structural leaf-span rule), `_parseCount` / `_extractOrderedNumbers` helpers. Mirror these in any new site module.

## Testing

- **Unit tests:** Jest + jsdom. Located in `tests/unit/`. 18 suites, 653 tests. Mock Chrome APIs via `tests/setup.js`. Coverage via `npm run test:coverage`.
...
- **Page-info format** — When `includeMetadata` is on, `ContentScript.addMetadataHeader(markdown, metadata, format)` emits either an inline bold key-value block (`format === 'header'`, see `_buildInlineHeader`) or YAML frontmatter (`format === 'yaml'`, see `_buildYamlFrontmatter`). Both formats carry the same minimal data shape: `Title`, `URL`, `Date`. Inline mode uses `**Key:** value` lines with markdown hard-breaks (`  \n`) so each key sits on its own line; the value `'header'` is preserved as the schema key for back-compat but the format is no longer an H1 — that change fixed the prior "double H1" bug where site-action formatters and the metadata prepend were both emitting H1s. Date is formatted as local `YYYY-MM-DD HH:mm` via `_formatLocalDateTime` — local time, not UTC, since UTC-date-via-`split('T')[0]` showed the wrong day for evenings west of UTC; 24h with no seconds since seconds rarely matter for "what time of day did I save this." YAML mode quote-escapes the title (`\\` and `"`), normalizes newlines, emits `url`/`date` unquoted (date as a YAML string scalar — most note tools accept this). Compatible with Obsidian, Logseq, Bear, Hugo, Jekyll. The popup checkbox stays as on/off; the format itself is options-page only (segmented control labels: "Inline" / "Frontmatter"). Domain was deliberately dropped from both formats — redundant against URL.
...
- **Output dispatch** — `dispatchOutput(markdown, metadata, modeOverride)` in background routes to `copyToClipboard()` or `saveAsFile()`. The optional `modeOverride` ('clipboard' | 'file') wins over the `outputMode` pref when supplied — the popup passes it on every action so Copy and Save are explicit per-click without rewriting the user's preferred default. Paths that don't pass an override (context-menu text selection, content-script `selectionComplete`) fall through to the pref. All output paths go through this single dispatcher. File saves call `BackgroundScript.generateFilename(metadata, prefs)` which delegates to `FilenameTemplate.formatFilename(template, style, { title, url, date })` — see `src/utils/filename-template.js` for the token set (`{title} {host} {domain} {path} {slug} {date[:fmt]} {time[:fmt]} {datetime[:fmt]}`), Moment-style date format tokens, pipe-filter syntax (`{title|max:N}`, `{slug|default:VALUE}`, chainable left-to-right), and pipeline (expand → apply default-title-cap → apply pipe filters → style transform → sanitize → truncate to 200 chars → append `.md`).
- **Site module interface** — Each site module exports a registration object with `id`, `hostnames` (matched against `URL.hostname`), `contentTypes` (each with `id`, `label`, `icon`, and `pathPatterns: RegExp[]` — the URL paths where this content type can be extracted), `Extractor` class, and `Formatter` class. `SiteRegistry` provides `detect(url)` (hostname → site), `getById(id)`, and `applicableContentTypes(site, url)` (filters `contentTypes` by `pathPatterns` matching `URL.pathname`; content types without `pathPatterns` are treated as always-applicable). The popup uses `applicableContentTypes` to render only the rows that fit the current URL — on `x.com/home` no rows show; on `/{user}/status/{id}` all three X content types show because the popup can't disambiguate without DOM. X/Twitter's `XExtractor` returns structured data objects (TweetData, ThreadData, ArticleData), `XFormatter` converts them to markdown.
- **Site formatter `filenameTitle(contentType, data)`** — Each site formatter exposes a method that returns a clean filename-safe title (or `null` to fall through to `document.title`). Patterns: X tweet → `X Post by @{handle}`; X thread → `X Thread by @{handle}`; X article → `data.title` (the `twitter-article-title` value); Claude/Grok conversation → `Claude — {title}` / `Grok — {title}` (or `Claude Conversation` / `Grok Conversation` when title is empty). `ContentScript.extractSiteContent` calls this and overrides `metadata.title` before returning, so file saves use the clean title. Without this, X tweets saved as `Riley Brown on X- -{280 chars of post text}.md` because `document.title` stuffs the post body into the OG title.
...
- **X quote tweets and link cards** — Both render as embedded blocks below the tweet body and need to be excluded from outer-tweet field extraction. Quote tweets sit inside a `[role="link"][tabindex="0"]` wrapper containing a nested `[data-testid="User-Name"]` (`_findQuoteContainer`); link cards sit inside `[data-testid="card.wrapper"]` with a single anchor wrapping image + title and a sibling div carrying the canonical `From {domain}` text (`_extractCard`). `_extractAuthor`, `_extractText`, `_extractTimestamp`, and `_extractMedia` all accept an `exclude` parameter (single element or array) so the focal tweet's fields skip over both subtrees — without this, the focal `<time>` silently inherits the quoted tweet's timestamp (the quote's `<time>` always appears first in DOM order) and the card image double-lists in `media`. Quote tweets get their own card extraction inside `_parseQuoteContainer` so a quoted post's card renders as part of its blockquote.
- **X "Show more" auto-expansion** — Site modules can implement an optional async `prepareForExtraction(contentType, doc, url)` method that the content script awaits before calling `extract()`. X uses it to click `[data-testid="tweet-text-show-more-link"]` buttons in scope (focal tweet for `single-tweet`, all chain tweets for `thread`) and poll for their removal — X inlines the full body via API on click and removes the button. 5s timeout per button (parallel `Promise.all`); on timeout we proceed with whatever's in the DOM. Without this, long tweets (>~280 chars) silently captured only the truncated prefix.
- **X thread chain detection** — `_collectThreadChain(tweets, focal, focalHandle)` walks DOM-forward and DOM-backward from the focal, gathering a contiguous run of same-author tweets. Skips empty `<article>` wrappers (X uses these for the reply-composer divider — they have no `User-Name` and shouldn't break the chain), but stops hard at the first real tweet whose author differs from the focal's. Used by `extractThread` (output) and `prepareForExtraction` (Show-more scope). The previous all-same-author rule swept up author replies-to-commenters from the discussion section below the thread; the chain rule excludes them by stopping at the first commenter (the boundary between thread and replies).
- **URL-shaped link label cleanup** — In `_formatTweetLink`, when the display label starts with `http` (i.e., it's an inlined URL link, not a mention or hashtag) and ends with `…` or `...`, the trailing ellipsis gets stripped via `_stripUrlEllipsis` since X uses it as visual chrome on truncated long-URL display even when the underlying href is intact. The t.co fallback path (where the real URL is unrecoverable) keeps its `…` since the ellipsis is meaningful chrome there.
- **X Article path** — X Articles live at three URL forms: `/i/article/{id}` (URL regex match), `/{user}/status/{id}` (canonical, DOM-detected), and `/{user}/article/{id}` (alternative, DOM-detected). All three serve the same `[data-testid="twitterArticleReadView"]` body, so detection uses URL regex first then falls back to a DOM testid check. `extractArticle` uses a layered scope: the broader `pageContainer` (`article[role="article"]`) holds author / `<time>` / engagement chrome, the read view holds the article-specific content, and `[data-testid="twitterArticleRichTextView"]` holds *just the body* (excludes engagement chrome). Title comes from `[data-testid="twitter-article-title"]` (the page-level og:title is the noisy `Author on X: "..." / X` wrapper). Cover image is the first `[data-testid="tweetPhoto"]` inside the read view but outside the body. Engagement (replies/reposts/likes/views) lives on the `pageContainer` outside the read view and gets pulled into `data.engagement`; the formatter renders it as an emoji footer at the very end of the article (matching the tweet convention). Body HTML is run through `_sanitizeArticleBody` which executes a pipeline of normalization passes — order matters where noted:
  1. **`_promoteInlineBold`** — wraps `<span style="font-weight: bold">` (or numeric ≥600) in `<strong>` so Turndown emits `**…**`. X uses inline styles for emphasis instead of `<strong>`/`<b>`, so without this every bold passage drops to plain text. Skips elements inside headings or already-strong ancestors.
  ...
  4. **`_sanitizeCodeBlocks`** — strips `[data-testid="markdown-code-block"]` chrome (visible language-label span + Copy button) leaving just `<pre><code class="language-X">`.
  5. **`_replaceVideoWithPoster`** — swaps `<video poster="…" src="…">` for `<img alt="GIF" src="{poster}">`. Turndown has no default `<video>` rule and would drop the embed entirely. **Also dedupes**: X renders animated GIFs as both a still-preview `<img>` AND a `<video poster=...>` with the same URL; this pass removes any sibling `<img>` whose `src` matches the poster before substituting, otherwise the same media URL appears twice in markdown (once with `alt="GIF"`, once empty).
  6. **`_unwrapMediaLinks`** — unwraps `<a href="/.../media/...">` link wrappers around inline tweetPhotos and video components (in-app media-viewer routes, useless in saved markdown).
  7. **`_stripVideoLabels`** — strips bare `<span>GIF</span>` / `<span>Video</span>` badge spans inside `videoComponent`/`videoPlayer`.
  8. **`_inlineMentionWrappers`** — unwraps the block-level `<div>` X wraps inline mentions in (`<div><a href="...">@user</a></div>` → `<a href="...">@user</a>`) so paragraphs flow as one block.
  9. **`_cleanMentionUrls`** — strips the leading `@` from mention URLs (`/@user` → `/user`; displayed text stays `@user`).
  Blockquotes (`<blockquote class="longform-blockquote">`, used for pull quotes / call-outs) work natively through Turndown's default rule even with the Draft.js `<div data-block><span data-text>` wrapping inside, so no sanitize step is needed.
- **Popup auto-detection is URL-only** — `SiteRegistry.detect(url)` checks hostname against registered site `hostnames`, called directly in popup (no background round-trip). `SiteRegistry.applicableContentTypes(site, url)` then filters the site's content types by `pathPatterns` against the URL's pathname. The popup hides the "Available on …" divider entirely when nothing applies (e.g. `x.com/home`, `claude.ai/new`, profile pages); on URLs that match a single content type only that row renders; on `/status/` URLs all three X rows show because we can't disambiguate without DOM. Wrong content type still gives a clear error message rather than pre-detecting via DOM round-trip. The popup also refuses to restore a remembered content type if it isn't applicable on the current URL — so picking "Thread" on a `/status/` page and then opening the popup on `/home` lands on Page-content, not a stale Thread selection.
